...
2. Find out **how much time has passed** in transit in hours (Aₜ), and compare with the **original total time** for the trip (Tₜ). Calculate this **percentage** (Pₜ) with this formula:

.. math::

    P_t = \frac{A_t-T_t}{T_t} * 100

3. Find out how much **partial Δv** you’ve actually spent during the transited trip (Δvₚ), which is a percentage of the original Δvₒ cost. The formula is:

.. math::

    \Delta v_p = \Delta v_o * \frac{P_t}{100}

4. Decide the **desired deceleration** for your maneuver in *G's* (d). Take into consideration the safety of passengers on board. A higher value in Gs will decrease the amount of time required for the maneuver, but it will increase its Δv cost.
5. Determine the **total time required** for the maneuver until reaching a full stop (Mₜ). The formula is:
...
.. math::
    ...
    \Delta v_n = \Delta v_o * d

7. Determine the **total Δv cost** (Δvₜ) (*partial* Δv cost + *maneuver* Δv cost) by doing:

.. math::

  \Delta v_t = \Delta v_p + \Delta v_n
